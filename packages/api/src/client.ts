...
export class ApiClient {
  ...
  async listPlans(
    params?: { q?: string; tags?: string[]; status?: string }
    ...
    const path = qs ? `/api/plans?${qs}` : "/api/plans";
  ...
  async updatePlan(
    ...
    if (!res.ok) {
      ...
      if (res.status === 409) {
        const conflictErr: Error & { currentVersion?: number } = new Error(
          err.error || "Conflict"
        );
  ...
  async setCommentResolved(
    ...
    const res = await this.request(
      `/api/plans/${planId}/comments/${commentId}`,
      {
        method: "PATCH",
  ...
  async getVersions(
    planId: string
  ): Promise<{ currentVersion: number; versions: VersionSummary[] }> {
  ...
  async getVersion(planId: string, versionId: string): Promise<VersionDetail> {
    const res = await this.request(
      `/api/plans/${planId}/versions/${versionId}`
    );
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  async getDiff(
    ...
    fromId: string,
    toId: string = "current"
  ): Promise<{ diff: DiffLine[] }> {
    const res = await this.request(
      `/api/plans/${planId}/diff?from=${fromId}&to=${toId}`
    );
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
